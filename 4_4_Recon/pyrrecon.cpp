...
//#pragma HLS unroll
			...
			if (coefIdx < MEMSIZE)
				...
				valSum = valMul;
			mem[c][idx] = valSum;*/
			//printf("%d %d %d %f %f %f\n", coefIdx, i,idx, val.real().to_float(), coef.to_float(),valMul.real().to_float());
...
//			mem[c][idx] = valMul;
			...
			if (coefIdx < MEMSIZE) {
				...
				mem[idx] += valMul;
			//printf("%d %d %d %f %f  %f     %f \n", coefIdx, i,idx, val.real().to_float(), coef.to_float(),valMul.real().to_float(),mem[idx].real().to_float());
		//}
		i++;
		l += (nlimit == i);
		i = (nlimit == i)?0:i;
...
void cmifft( hls::stream< t_recon_complex>  &DFT,
...
#pragma HLS STREAM variable=IMOutMem  depth=1 dim=1
...
#pragma HLS DATAFLOW
	for(int i=0;i<MEMSIZE;i++) {
#pragma HLS PIPELINE
		t_recon_complex t = DFT.read();

		cmpxIFFTIn    val;
		val.real().range(23,0) = t.real().range(23, 0);
		val.imag().range(23,0) = t.imag().range(23, 0);
		DFTOutMem[i] = val;
...
void pyr_recon_ifft(hls::stream< t_recon_complex>  DFTOut[reconC],  hls::stream<  PIXEL_RAW>  &RGBOut) {

#pragma HLS data_pack variable=DFTOut
#pragma HLS data_pack variable=RGBOut

	hls::stream< ifft_out_t>  sigOut[reconC];

#pragma HLS DATAFLOW
	cmifft(DFTOut[0], sigOut[0]);
	cmifft(DFTOut[1], sigOut[1]);
	cmifft(DFTOut[2], sigOut[2]);

	for(int i=0;i<512;i++ ){
#pragma HLS PIPELINE
		ifft_out_t Rs = sigOut[0].read() <<10 ;
